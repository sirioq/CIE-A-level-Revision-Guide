...
\begin{tikzpicture}[scale=0.9]
  ...
  \node[darkblue, font=\small, anchor=west] at (2.15,2.5) {$I$ (upward)};
  % Force arrow
  \draw[->, pinkframe, very thick] (2,.8) -- (0.6,.8)
    node[anchor=east, font=\small, pinkframe] {$F$};
  % B label
  \node[darkgray, font=\small] at (3.5,.5) {$B$ (into page)};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=1.4,
  ...
  % Caption
  \node[darkblue, font=\small\bfseries] at (0,-1.9,0) {Magnetic field around a straight wire};
\end{tikzpicture}
...
\def\ytop{0.9}
\def\ybot{-0.9}
\pgfmathsetmacro{\xend}{\xstart+\N*\dx}
...
\begin{tikzpicture}[scale=1.4]
  ...
  % ── Dot symbols on top ─────────────────────────────────────
  \foreach \i in {0,1,...,\N}{
    \draw[darkblue, thick] (\xstart+\i*\dx, \ytop) circle [radius=0.09];
    \filldraw[darkblue]    (\xstart+\i*\dx, \ytop) circle [radius=0.035];
  }

  % ── Cross symbols on bottom ────────────────────────────────
  \foreach \i in {0,1,...,\N}{
    \pgfmathsetmacro{\cx}{\xstart+\i*\dx}
    \draw[darkblue, thick] (\cx, \ybot) circle [radius=0.09];
    \draw[darkblue, thick]
      (\cx-0.063, \ybot-0.063) -- (\cx+0.063, \ybot+0.063)
      (\cx-0.063, \ybot+0.063) -- (\cx+0.063, \ybot-0.063);
  }

  % ── Front semi-circles (solid) ─────────────────────────────
  \foreach \i in {0,1,...,\N}{
    \pgfmathsetmacro{\cx}{\xstart+\i*\dx}
    \draw[midblue, line width=1.4]
      (\cx, \ybot) arc (270:90: 0.35 and 0.9);
  }

  % ── Back semi-circles (dashed) ─────────────────────────────
  \foreach \i in {0,...,\N}{
    \pgfmathsetmacro{\cx}{\xstart+\i*\dx}
    \pgfmathsetmacro{\lw}{ifthenelse(\i==0, 1.4, 1.0)}
    \draw[midblue, dashed, line width=\lw]
      (\cx, \ytop) arc (90:-90: 0.35 and 0.9);
  }

  % ── MIDDLE field line ──────────────────────────────────────
  \draw[midblue, line width=1.6,
    postaction={decorate, decoration={markings,
      mark=at position 0.5 with {\arrow[midblue]{Stealth[length=9pt, width=6pt]}}}}]
    (\xstart-\ext, 0) -- (\xend+\ext, 0);

  % ── TOP field line: internal ───────────────────────────────
  \draw[midblue, line width=1.6,
    postaction={decorate, decoration={markings,
      mark=at position 0.5 with {\arrow[midblue]{Stealth[length=9pt, width=6pt]}}}}]
    (\xstart, 0.45) -- (\xend, 0.45);
  % external loop
  \draw[midblue, line width=1.4,
    postaction={decorate, decoration={markings,
      mark=at position 0.5 with {\arrow[midblue]{Stealth[length=9pt, width=6pt]}}}}]
    (\xend, 0.45)
    .. controls (\xend+\ext, 0.45) and (\xend+\ext, \lh)
    .. (\xmid, \lh)
    .. controls (\xstart-\ext, \lh) and (\xstart-\ext, 0.45)
    .. (\xstart, 0.45);

  % ── BOTTOM field line: internal ────────────────────────────
  \draw[midblue, line width=1.6,
    postaction={decorate, decoration={markings,
      mark=at position 0.5 with {\arrow[midblue]{Stealth[length=9pt, width=6pt]}}}}]
    (\xstart, -0.45) -- (\xend, -0.45);
  ...
  % ── N and S pole labels ────────────────────────────────────
  \node[darkblue, font=\large\bfseries] at (\xend+0.55, .15) {N};
  \node[darkblue, font=\large\bfseries] at (\xstart-.5, .15) {S};

  % ── B label ───────────────────────────────────────────────
  \node[midblue, font=\small\bfseries] at (\xmid, 0.65) {$B$};

   % Caption
  \node[darkblue, font=\small\bfseries] at (2.2,-2.5,0) {Magnetic field around a Solenoid};

% =============================================================================
% topics_14_15_16.tex — Content only. No \documentclass.
% Compile standalone via topics_14_15_16_wrapper.tex
% Compile as part of series via main.tex (one level up)
% =============================================================================

\setcounter{section}{0}   % ensure sections start at 1 regardless of context
...
\maketitlepage
  ...
  {%
    \begin{itemize}[leftmargin=1cm, itemsep=2pt, label=\textbullet,
                    topsep=4pt]
...
\begin{itemize}
  \item $0\,{}^{\circ}\text{C}$ — the ice point (pure water–ice equilibrium at
        $1\,\text{atm}$)
  \item $100\,{}^{\circ}\text{C}$ — the steam point (pure water–steam equilibrium at
        $1\,\text{atm}$)
\end{itemize}
...
A temperature \emph{difference} $\Delta\theta$ in ${}^{\circ}$C equals the same
numerical difference $\Delta T$ in K, so conversion is not needed for differences —
but is always required for absolute values in $pV = nRT$.
\end{tipbox}
...
\begin{itemize}
  ...
  \item \textbf{Specific latent heat of vaporisation} $L_v$ — liquid
        $\leftrightarrow$ gas
\end{itemize}
...
forgetting to split a calculation into separate stages: warming, change of state, then
further warming.
\end{warnbox}
...
\begin{tipbox}
For a fixed mass of gas changing conditions:
\[
...
\begin{enumerate}
  ...
  \item Time between successive collisions with the same wall:
        $\Delta t = 2L/c_x$.
  \item Force from one molecule: $F = mc_x^2/L$.
...
Both are equivalent — always state your sign convention before substituting.
\end{tipbox}
...
\begin{tabular}{p{2.6cm}p{3.8cm}p{5.8cm}}
\toprule
...
\begin{tabular}{p{6.8cm} p{5.2cm} p{1.6cm}}
\toprule
...
\begin{enumerate}
  ...
  \item \textbf{Check your answer}: correct units, sensible order of magnitude,
        correct sign.
\end{enumerate}
...
\begin{itemize}
  \item Split mixed heating problems into separate stages: warming + change of
        state + further warming.
  \item Remember $L_v \gg L_f$: for water,
        $L_v \approx 2.26\times10^6\,\mathrm{J\,kg}^{-1}$ vs
        $L_f \approx 3.34\times10^5\,\mathrm{J\,kg}^{-1}$.
  \item Calorimetry: set $IVt = mc\Delta T$; watch for heat losses.
\end{itemize}
...
\begin{itemize}
  ...
  \item The kinetic theory derivation is examinable — practise the 3-step argument
        (momentum change $\to$ force $\to$ pressure, then extend to 3-D).
  \item R.m.s.\ speed $\propto \sqrt{T}$: doubling $T$ in kelvin multiplies
        $c_{\mathrm{r.m.s.}}$ by $\sqrt{2}$, \emph{not} by 2.
\end{itemize}

\subsubsection{Topic 16 — Thermodynamics}
\begin{itemize}
  \item Be rigorous about sign conventions: state them explicitly before
        substituting.
  \item For an isothermal process with an ideal gas, $\Delta U = 0$ always.
  \item Work done \textbf{on} the gas is $+W$ in CIE's first law convention.
\end{itemize}
...
\noindent\textbf{Question 1}\quad An electric heater of power 60\,W heats 0.50\,kg of
water from $20\,{}^{\circ}$C to $70\,{}^{\circ}$C in 175\,s. Calculate the specific
...
\bigskip
\noindent\textbf{Question 2}\quad A gas occupies $2.0\times10^{-3}\,\mathrm{m}^3$ at a
pressure of $1.5\times10^5\,\mathrm{Pa}$ and temperature $27\,{}^{\circ}$C. Calculate
...
\bigskip
\noindent\textbf{Question 3}\quad Calculate the r.m.s.\ speed of nitrogen molecules
...
\bigskip
\noindent\textbf{Question 4}\quad A gas is compressed. The work done \emph{on} the gas
is 450\,J and at the same time 200\,J of heat is removed from the gas. Find the change
in internal energy.
...
$c_{\mathrm{r.m.s.}}$ by factor $\sqrt{2} \approx 1.41$ [1]; because average KE
increases [1].
...
on the gas during compression increases internal energy [1].

\medskip
\textbf{Q8.}
From kinetic theory: $pV = \tfrac{1}{3}Nm\langle c^2\rangle$ [1];
from ideal gas: $pV = NkT$;
equating and simplifying: $\tfrac{1}{2}m\langle c^2\rangle = \tfrac{3}{2}kT$ [1];
this equals the average translational kinetic energy of one molecule [1].
\end{markbox}
...
\midrule
\multicolumn{2}{l}{\bfseries\color{cieprimary} Topic 14 — Temperature} \\
...
\midrule
\multicolumn{2}{l}{\bfseries\color{cieprimary} Topic 15 — Ideal Gases} \\
...
\midrule
\multicolumn{2}{l}{\bfseries\color{cieprimary} Topic 16 — Thermodynamics} \\

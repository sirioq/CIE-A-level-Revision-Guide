...
\begin{tikzpicture}[scale=1.0]
  ...
  \node[darkblue, font=\small\bfseries, anchor=south] at (0,1.15) {Proton ($+e$)};
  % u quark top-left
  ...
  \node[darkblue, font=\small\bfseries, anchor=south] at (3.5,1.15) {Neutron ($0$)};
  % u quark top
  ...
  \node[font=\footnotesize, anchor=west] at (5.75, 0.45) {u quark ($+\tfrac{2}{3}e$)};
  \filldraw[fill=pinkframe!30, draw=pinkframe] (5.5,-0.1) circle (0.2);
  \node[font=\footnotesize, anchor=west] at (5.75, -0.1) {d quark ($-\tfrac{1}{3}e$)};
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=1.0, every node/.style={font=\small}]
  % Neutron: udd
  \node[darkblue, font=\small\bfseries] at (-2.5, 1.8) {Neutron (udd)};
  ...
  % Arrow
  \draw[->, very thick, darkblue] (-1.2, 0.8) -- (0.2, 0.8)
    node[midway, above, font=\footnotesize] {$\beta^-$ decay};

  % Proton: uud
  \node[darkblue, font=\small\bfseries] at (1.5, 1.8) {Proton (uud)};
  ...
  \draw[->, pinkframe, thick] (2.7,0.8) -- (3.8,1.4);
  \node[pinkframe, anchor=west] at (3.85,1.4) {$\mathrm{e}^-$};
  \draw[->, darkgray, thick] (2.7,0.7) -- (3.8,0.1);
  \node[darkgray, anchor=west] at (3.85,0.1) {$\bar{\nu}_e$};

  % Changed quark annotation
  \draw[accentgold, dashed, thick, ->, rounded corners]
    (-2.1, 0.22) .. controls (-1.0,-0.5) .. (1.1, 0.22);
  \node[accentgold, font=\footnotesize] at (-0.5,-0.65) {d $\to$ u};
\end{tikzpicture}
\end{center}

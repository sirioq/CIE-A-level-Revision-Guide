...
\chapteropener{13}{Gravitational Fields}{%
  Newton's Law of Gravitation $\cdot$ Gravitational Field Strength $\cdot$
  Gravitational Potential $\cdot$ Orbital Motion \& Kepler's Third Law $\cdot$
  Escape Velocity}
...
\begin{keybox}{Interpreting the Gradient}
$g$ is the \textbf{negative gradient} of the potential--distance graph. On a $\phi$--$r$ graph:
\begin{itemize}[itemsep=2pt]
  \item A steeper gradient $\Rightarrow$ stronger field (larger $g$)
  \item The gradient becomes shallower as $r$ increases $\Rightarrow$ $g$ decreases
  \item The area under a $g$--$r$ graph gives the change in potential $\Delta\phi$
\end{itemize}
\end{keybox}

...
\begin{circuitikz}[scale=1.1]
  % Simple capacitor symbol with charge labels
  \draw (0,0) to[C, l=$C$] (3,0);
  \node[darkblue, font=\small] at (0.95, 0.55) {$+Q$};
  \node[darkblue, font=\small] at (2.05, 0.55) {$-Q$};
  \draw[darkblue, ->, >=stealth] (1.5,-0.7) -- (1.5,-0.25)
    node[midway, right, font=\small] {$V$};
\end{circuitikz}
\end{center}
...
\begin{tikzpicture}[xscale=2.8, yscale=2.2]
  ...
  \node[pinkframe, font=\small] at (0.38, 0.28) {$W = \tfrac{1}{2}Q_0 V_0$};
...
\begin{center}
\begin{circuitikz}[scale=1.1]
  \draw (0,2) to[C, l=$C$, v=$V_0$] (0,0)
        -- (3,0)
        to[R, l=$R$] (3,2)
        -- (0,2);
  \draw[dashed, darkgray] (1.5,2) to[nos] (1.5,1.2);
  \node[darkgray, font=\small] at (1.5,0.85) {switch};
\end{circuitikz}
\hspace{1.5cm}

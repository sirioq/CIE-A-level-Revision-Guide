...
\begin{axis}[
  ...
  \node[darkblue, font=\small\bfseries, anchor=north] at (axis cs:9.5,-0.1)
    {Half-wave rectification};
  \node[anchor=west] at (axis cs:19,0) {$t$};
\end{axis}
\end{tikzpicture}
\vspace{0.8em}
...
\begin{circuitikz}[european resistors, scale=1.2,
  ...
  \coordinate (RIGHT) at (6, 3);

  % Four diodes — conventional current flows: LEFT->TOP, BOT->LEFT, TOP->RIGHT, RIGHT->BOT
  \draw (LEFT)  to[full diode, l=$D_3$, *-*] (TOP);
  \draw (LEFT)   to[diode, l_=$D_4$, *-*] (BOT);
  \draw (TOP)   to[diode, l=$D_1$, *-*] (RIGHT);
  \draw (BOT) to[full diode, l_=$D_2$, *-*] (RIGHT);

  % AC source on the left (LEFT to BOT via external wire)
  \draw (TOP) -- (-1,6) ;
  \draw (BOT)  -- (3, -.5) -- (-1, -.5);
  \draw (-1, -.5) to[sV, l=a.c.] (-1, 6);

  % Load resistor on the right (TOP to BOT via external wire)
  \draw (RIGHT)   -- (7.5, 3) ;
  \draw (LEFT)   -- (0, -1.2) -- (7.5, -1.2);
  \draw (7.5, 3) to[R, l=$R_L$, *-*] (7.5, -1.2);

  % +/- output labels
  \node[darkblue, anchor=south] at (7.5, 3)  {$+$};
...
\begin{axis}[xlabel={$t$},xmin=-3,xmax=21,
             ylabel={$V$},ymin=0,ymax=2,
             ytick=\empty,
             xtick=\empty,
             axis x line=center, 
             axis y line=left, enlargelimits=upper]
% draw sine functions
\addplot [dashed,thin,domain=-pi:6.8*pi,smooth]{-2*sin(deg(0.5*x))};
...
\addplot [midblue,thick,domain=\i*pi:{\j*pi+\arch}, ] {2*e^(-0.05*(x-\i*pi)};  % exponentially decay curves, not a line
\addplot [midblue,thick,domain={\j*pi+\arch}:\k*pi, smooth]{ 2*sin(deg(0.5*x))};
\addplot [midblue,thick,domain={\j*pi+\arch}:\k*pi, smooth]{-2*sin(deg(0.5*x))};
}
\end{axis}
\end{tikzpicture}
...
\begin{warningbox}{Common Mistake}
In exam questions, always link ripple size explicitly to the time constant $\tau = CR_L$ — just saying ``bigger capacitor'' without explanation will not earn full marks.
\end{warningbox}

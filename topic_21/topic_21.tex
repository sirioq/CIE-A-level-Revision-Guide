...
\begin{tikzpicture}[scale=1.15, every node/.style={font=\small}]
  \coordinate (T)   at (2.2, 3.0);
  \coordinate (Bot) at (2.2, 0.0);
  \coordinate (L)   at (0.0, 1.5);
  \coordinate (R)   at (4.4, 1.5);

  % Diodes as labelled arrows
  ...
  \draw (L)   -- (-1.2, 1.5);
  \draw (Bot) -- (-1.2, 0.0);
  \draw[darkblue, thick] (-1.2, 0.0) -- (-1.2, 1.5);
  \node[darkblue, font=\normalsize] at (-1.2, 0.75) {$\sim$};
  \node[darkblue] at (-1.2, -0.35) {a.c. input};

  % Load (right)
  \draw (T)   -- (4.4, 3.0) -- (5.8, 3.0);
  \draw (Bot) -- (4.4, 0.0) -- (5.8, 0.0);
  \draw[darkblue, very thick] (5.8, 0.0) -- (5.8, 3.0);
  \node[darkblue, anchor=west] at (5.9, 1.5) {$R_L$};
  \node[darkblue] at (5.8, -0.35) {d.c. output};

  % Junction dots
  \fill[darkblue] (T)   circle (2.5pt);
  \fill[darkblue] (Bot) circle (2.5pt);
  \fill[darkblue] (L)   circle (2.5pt);
  \fill[darkblue] (R)   circle (2.5pt);
\end{tikzpicture}
\end{center}

...
\begin{axis}[
  ...
  % Dashed unsmoothed full-wave
  \addplot[darkgray, dashed, thin, domain=-3:21, samples=400, smooth]
    {abs(1.2*sin(deg(0.5*x)))};

  % Smoothed output using same foreach pattern as user's example
  % Half-period = 2*pi ~ 6.28; peaks at x = pi, 3pi, 5pi, 7pi ~ 3.14, 9.42, 15.71
  % Start from x=0 already charged (capacitor pre-charged to peak)
  \addplot[midblue, thick, domain=0:{\arch}, samples=80, smooth]
    {1.2*exp(-0.05*x)};
  \foreach \i/\j/\k in {-1/0/1, 1/2/3, 3/4/5}{
    \addplot[midblue, thick, domain={\i*3.14159+\arch}:{\j*3.14159+\arch}, samples=80, smooth]
      {1.2*exp(-0.05*(x-\i*3.14159))};
    \addplot[midblue, thick, domain={\j*3.14159+\arch}:{\k*3.14159}, samples=80, smooth]
      {abs(1.2*sin(deg(0.5*x)))};
  }

  % Labels inside plot area
  \node[midblue,  font=\small] at (axis cs:18, 1.35) {smoothed};
  \node[darkgray, font=\small] at (axis cs:18, 0.55) {unsmoothed};

  % Ripple annotation
  \draw[<->, pinkframe, thin]
    (axis cs:{3.14159+\arch}, {1.2*exp(-0.05*\arch)})
    -- (axis cs:{3.14159+\arch}, {1.2})

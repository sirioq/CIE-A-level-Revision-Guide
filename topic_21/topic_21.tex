...
\begin{axis}[
  ...
  \addplot[midblue, thick, domain=0:19, samples=300, smooth]
    {max(0, 1.2*sin(deg(0.5*x)))};
  \node[darkblue, font=\small\bfseries, anchor=north] at (axis cs:9.5,-0.28)
    {Half-wave rectification};
\end{axis}
\end{tikzpicture}
...
% arch = x value where exponential decay meets the rising sine in next half-cycle
% Decay: 1.2*e^{-0.15*(x - peak)}; peak at x = pi/2 + n*pi (in units where omega=0.5, peaks at 3.14, 9.42, ...)
% Using same x-scale as rectification plots (omega=0.5, so period=4*pi~12.6, half-period~6.28)
\begin{center}
\begin{tikzpicture}
\pgfmathsetmacro{\arch}{4.3}% x-offset within half-cycle where decay meets sine
\begin{axis}[
  ...
  % Dashed unsmoothed full-wave background
  \addplot[darkgray, dashed, thin, domain=0:19, samples=400, smooth]
    {abs(1.2*sin(deg(0.5*x)))};

  % Smoothed output: for each half-cycle starting at i*2*pi/omega = i*4*pi (period), 
  % using half-period 2*pi (in these x-units period is ~12.57, half-period ~6.28)
  % Peaks of |sin(0.5x)| at x = pi, 3pi, 5pi, 7pi = 3.14, 9.42, 15.71, 21.99
  % Four segments: decay from each peak until sine overtakes, then ride sine to next peak
  \foreach \peakx/\nextpeakx in {3.14/9.42, 9.42/15.71, 15.71/21.99}{
    % Exponential decay from peak
    \addplot[midblue, thick, domain=\peakx:{\peakx+\arch}, samples=80, smooth]
      {1.2*exp(-0.12*(x-\peakx))};
    % Sine recharge up to next peak
    \addplot[midblue, thick, domain={\peakx+\arch}:\nextpeakx, samples=80, smooth]
      {abs(1.2*sin(deg(0.5*x)))};
  }

  % Labels
  \node[midblue,  font=\small, anchor=west] at (axis cs:16, 1.28) {smoothed};
  \node[darkgray, font=\small, anchor=west] at (axis cs:16, 0.60) {unsmoothed};

  % Ripple arrow on first decay segment
  \draw[<->, pinkframe, thin]
    (axis cs:{3.14+\arch}, {1.2*exp(-0.12*\arch)})
    -- (axis cs:{3.14+\arch}, {abs(1.2*sin(deg(0.5*(3.14+\arch))))})

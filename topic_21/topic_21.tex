...
\begin{axis}[
  ...
  \addplot[midblue, thick, domain=0:pi,     samples=100, smooth] {1.2*sin(deg(x))};
  \addplot[midblue, thick, domain=pi:2*pi,  samples=2]           {0};
  \addplot[midblue, thick, domain=2*pi:3*pi,samples=100, smooth] {1.2*sin(deg(x))};
  \addplot[midblue, thick, domain=3*pi:4*pi,samples=2]           {0};
  \addplot[midblue, thick, domain=4*pi:6.3, samples=100, smooth] {1.2*sin(deg(x))};
  \node[darkblue, font=\small\bfseries] at (axis cs:3.14,-0.25) {Half-wave rectification};
\end{axis}
\end{tikzpicture}
...
\begin{axis}[
  ...
  % All half-cycles made positive
  \addplot[midblue, thick, domain=0:pi,     samples=100, smooth] { 1.2*sin(deg(x))};
  \addplot[midblue, thick, domain=pi:2*pi,  samples=100, smooth] {-1.2*sin(deg(x))};
  \addplot[midblue, thick, domain=2*pi:3*pi,samples=100, smooth] { 1.2*sin(deg(x))};
  \addplot[midblue, thick, domain=3*pi:4*pi,samples=100, smooth] {-1.2*sin(deg(x))};
  \addplot[midblue, thick, domain=4*pi:6.3, samples=100, smooth] { 1.2*sin(deg(x))};
  \node[darkblue, font=\small\bfseries] at (axis cs:3.14,-0.25) {Full-wave rectification};
\end{axis}
\end{tikzpicture}
\end{center}
...
\begin{axis}[
  ...
  \addplot[darkgray, dashed, thin, domain=0:pi,     samples=100, smooth] { 1.2*sin(deg(x))};
  \addplot[darkgray, dashed, thin, domain=pi:2*pi,  samples=100, smooth] {-1.2*sin(deg(x))};
  \addplot[darkgray, dashed, thin, domain=2*pi:3*pi,samples=100, smooth] { 1.2*sin(deg(x))};
  \addplot[darkgray, dashed, thin, domain=3*pi:4*pi,samples=100, smooth] {-1.2*sin(deg(x))};
  \addplot[darkgray, dashed, thin, domain=4*pi:6.4, samples=100, smooth] { 1.2*sin(deg(x))};

  % Smoothed output: exponential decay from peak, recharged by next sine peak
  % Each half-cycle peak is at x = pi/2, 3pi/2, 5pi/2 ...
  % Decay constant chosen so ripple is visible but small
  \pgfmathsetmacro{\tau}{1.8}
  \addplot[midblue, thick, domain=pi/2:3*pi/2,   samples=150, smooth]
    {1.2*exp(-(x - pi/2)/\tau)};
  \addplot[midblue, thick, domain=3*pi/2:5*pi/2, samples=150, smooth]
    {1.2*exp(-(x - 3*pi/2)/\tau)};
  \addplot[midblue, thick, domain=5*pi/2:6.4,    samples=150, smooth]
    {1.2*exp(-(x - 5*pi/2)/\tau)};

  % Labels
  \node[midblue,  font=\small, anchor=west] at (axis cs:5.6, 1.10) {smoothed};
  \node[darkgray, font=\small, anchor=west] at (axis cs:5.6, 0.55) {unsmoothed};

  % Ripple annotation on second decay segment
  \draw[<->, pinkframe, thin]
    (axis cs:{3*pi/2}, {1.2*exp(-(3*pi/2 - pi/2 - pi)/\tau)})
    -- (axis cs:{3*pi/2}, 1.2)
    node[midway, right, pinkframe, font=\scriptsize] {ripple};
\end{axis}
\end{tikzpicture}
\end{center}
